...
describe('JSON configs', () => {
    test('adds an entry to an empty file', () => {
        const result = JSON.parse(upsertJsonMcpServer('', 'unity-mcp', 'node', ['x.js']));

        expect(result.mcpServers['unity-mcp']).toEqual({ command: 'node', args: ['x.js'] });
    });
    ...
    test('preserves other servers and unrelated keys', () => {
        ...
        const after = JSON.parse(upsertJsonMcpServer(before, 'unity-mcp', 'node', ['x.js']));
    ...
    test('removes only our entry', () => {
        const before = JSON.stringify({
            mcpServers: { other: { command: 'other' }, 'unity-mcp': { command: 'node' } },
        });

        const after = JSON.parse(removeJsonMcpServer(before, 'unity-mcp') as string);

        expect(after.mcpServers.other).toBeDefined();
        expect(after.mcpServers['unity-mcp']).toBeUndefined();
    });

    test('reports nothing to remove', () => {
        expect(removeJsonMcpServer(JSON.stringify({ mcpServers: {} }), 'unity-mcp')).toBeNull();
    });
});

describe('TOML configs', () => {
    test('appends a new block without disturbing the rest', () => {
        const after = upsertTomlMcpServer(CODEX_CONFIG, 'unity-mcp', 'node', ['H:/build/index.js']);

        // Everything that was there before is still there, byte for byte.
        expect(after.startsWith(CODEX_CONFIG.trimEnd())).toBe(true);
        expect(after).toContain('[mcp_servers.unity-mcp]');
    ...
    test('replaces an existing block in place', () => {
        const once = upsertTomlMcpServer(CODEX_CONFIG, 'unity-mcp', 'node', ['old.js']);
        const twice = upsertTomlMcpServer(once, 'unity-mcp', 'node', ['new.js']);

        expect(twice.match(/\[mcp_servers\.unity-mcp\]/g)).toHaveLength(1);
    ...
    test('replacing a middle block leaves the following one intact', () => {
        ...
        const withOurs = upsertTomlMcpServer(CODEX_CONFIG, 'unity-mcp', 'node', ['a.js']);
        const reordered = withOurs.replace(
            '[shell_environment_policy]',
            '[mcp_servers.unity-mcp]\ncommand = "stale"\nargs = []\n\n[shell_environment_policy]'
        );

        const after = upsertTomlMcpServer(reordered, 'unity-mcp', 'node', ['b.js']);
    ...
    test('removes only our block', () => {
        const withOurs = upsertTomlMcpServer(CODEX_CONFIG, 'unity-mcp', 'node', ['x.js']);
        const after = removeTomlMcpServer(withOurs, 'unity-mcp') as string;

        expect(after).not.toContain('[mcp_servers.unity-mcp]');
        expect(after).toContain('[mcp_servers.maya]');
    ...
    test('a sub-table belongs to its parent block', () => {
        ...
        const config = `[mcp_servers.unity-mcp]
command = "node"
args = []
...
`;
        ...
        const after = removeTomlMcpServer(config, 'unity-mcp') as string;
    ...
    test('reports nothing to remove', () => {
        expect(removeTomlMcpServer(CODEX_CONFIG, 'unity-mcp')).toBeNull();
    });
    ...
    test('handles a block at the end of the file', () => {
        ...
        expect(after).not.toContain('unity-mcp');
    });

    test('quotes arguments so Windows paths survive', () => {
        const after = upsertTomlMcpServer('', 'unity-mcp', 'node', ['C:\\Users\\x\\build\\index.js']);
