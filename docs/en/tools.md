...
| `inspect_read` | safe | Read a serialized property. Omit `component_type` to address the GameObject itself. An array carries its own values in `elements` alongside its length, up to the first 20, with `elementsShown` when it was cut. Elements that are structs are left out |
