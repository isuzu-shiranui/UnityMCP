...
This page lists the Editor's tools, one table per group. [Back to the README](../../README.en.md)

Available tools depend on the connected project's packages. Timeline and Recorder tools need `com.unity.timeline` and `com.unity.recorder`, respectively. `test_run` and `test_results` need `com.unity.test-framework`.

The idempotency column says whether a call may be retried automatically after a connection failure. Built-in tools use `safe` only for read-only, retryable operations. `reflect_read`/`gpu_readback` can invoke arbitrary getters, and `capture_screenshot` can write files, so these are `unsafe`.

On Unity 6.5 and later, `instanceId` comes back as a JSON string rather than a number, because a 64-bit EntityId does not survive a JSON number. The `instance_id` argument accepts either.
...
| `test_results` | safe | The current or most recent run (readable while it runs). `total` counts the tests that ran, leaving out skipped tests, and `ranAny` says whether any ran |
| `scene_browse_hierarchy` | safe | Walk the hierarchy. Emits `path`, which every editing tool takes. A filtered walk also returns the parents leading down to each match, but not a match's own children unless they match too: filtering by a parent's name answers with that parent alone. `childrenNotShown` counts the children a node is missing, whether the filter dropped them or `max_depth` stopped the walk above them, so a node without that key is a leaf and a node with it has more underneath. A `fields` allowlist keeps it. `missing_scripts: true` returns only objects carrying a component whose script cannot be resolved, and `missingScripts` on each object counts them. A key missing from a node is at its default: active true, tag Untagged, layer Default, no missing scripts. `object_path` starts the walk at that object instead of the scene roots, so asking what is under a known object does not mean taking the whole scene. Every reply carries a `snapshotId`; pass one back as `since` and the reply is what differs from that state instead of the tree. A snapshot does not survive the Editor reloading its scripts; one it no longer holds is answered with the tree under a `sinceExpired` rather than an error. A snapshot from a reply that `limit` or `offset` cut short cannot be diffed against |
| `scene_list` | safe | Open scenes, and the scenes in the build settings |
| `inspect_read` | safe | Read a serialized property. A C# name such as `mass` or `localPosition.x` resolves to its serialized path when unambiguous; errors include nearby serialized path candidates. Without `component_type`, it tries the GameObject first, then a component that uniquely matches the property. An array carries its own values in `elements` alongside its length, up to the first 20, with `elementsShown` when it was cut. Elements that are structs are left out. `asset_path` reads the contents of a prefab or the main asset at that path. For a prefab child, give `object_path` relative to the prefab root, leaving out the root name. `component_type` takes a short or full type name |
| `inspect_list` | safe | Discover property paths. Pass `property_path` to describe one property, using its serialized path or an unambiguous C# name. With no property named, omit `component_type` to list the components on the GameObject, and add `detail: "full"` for every component's properties in one call rather than one call per component. `asset_path` reads the contents of a prefab or the main asset at that path. For a prefab child, give `object_path` relative to the prefab root, leaving out the root name. `component_type` takes a short or full type name |
...
| `play_mode_status` | safe | Playing / paused / compiling. `pending` holds `play` until the Editor has entered play mode, reached the requested pause state and completed a frame, or `stop` until it has left play mode. A refusal clears `pending` and leaves the action and reason in `refused`, which is otherwise null. The request survives a domain reload |
| `project_assemblies` | safe | Loaded assemblies |
| `project_packages` | safe | UPM packages |
| `capture_screenshot` | unsafe | Game or Scene view, or an Editor panel. A panel is read off the screen, so anything covering the Editor is in the image. It focuses the window first. `save_path` writes it to disk, creating directories and overwriting. Name the subject in `focus` and the call is refused, rather than rendered, when that object is outside what the camera sees |
| `job_status` | safe | State and result of a job id returned by a slow call. A record is kept for ten minutes after a job finishes and none survives a domain reload, so a job that ran to completion and one a reload cut short both come back as not found and cannot be told apart. A job whose own work reloads the domain - a settings write that saves assets, a recompile, entering Play Mode - reaches that every time, so read back what the work would have changed rather than polling for it |
...
| `memory_usage` | safe | What the loaded objects weigh, largest first, **split by whether the project owns them**. That split is what makes the answer usable: an Editor holds its own render targets and icon atlases, and those are routinely the largest textures in memory while telling a developer nothing. The reply leads with a row per type, then names the heaviest individual assets with their paths. These are the sizes of **what is loaded now**, not of the files on disk and not of what a built player would hold - the Editor keeps textures in forms a build does not, so read the figures as a ranking rather than as a budget |
| `ui_hit_test` | safe | What a click at a screen point actually reaches, and what is in the way. This is the answer to "the button does nothing": the reply lists every uGUI element under the point in the order the event system sees them, so the first one takes the click, and naming the element you meant in `object_path` gets you what covers it. It also lists elements under the point that cannot be hit at all, each with the reason - Raycast Target off, an inactive object, a CanvasGroup with Blocks Raycasts off, a Canvas with no GraphicRaycaster. **It needs play mode**, because `EventSystem.current` is only set while the game runs; outside it every point reports nothing under it, which reads exactly like a correctly-answered question about a broken UI. uGUI only; UI Toolkit is not covered. The tool is absent in a project without com.unity.ugui |
| `ui_click` | unsafe | Press a uGUI element through the EventSystem in play mode. The Editor does not need focus. Name exactly one target with `object_path`, `instance_id`, `text` or `position`. Text must match the label or name of one active, interactable Selectable; a position is `[x, y]` measured from the bottom left, or fractions from 0 to 1 with `normalized: true`. A covered target is refused. The reply waits `wait_frames` frames (one by default), stepping them if paused. `text_changes` is on by default: it compares up to 2000 Text or TMP_Text components and reports up to 50 changes. Selectables and pointer handlers receive the click; code that reads the device directly does not. Drag and scroll are not sent |
| `definitions_list` | safe | What defined tools loaded, and why one did not |

## Authoring

One call collapses into a single undo step wherever a tool declares an `UndoGroup`. In authoring those are the eight `gameobject_` tools, `inspect_write`, `prefab_create`, `prefab_instantiate`, `animator_create`, `animation_clip_create` and the ten `animator_` editing tools, twenty-three in all. In rendering they are `material_set`, `material_create` and `scene_settings`. In builds it is `project_settings`. In Timeline and Recorder they are the seven `timeline_` tools and `recorder_add_track`.
...
| Tool | Idempotency | Purpose |
|---|---|---|
| `gameobject_create` | unsafe | Create an object, optionally a primitive. The new object becomes the selection. A primitive arrives with a collider the way Unity's own menu makes one; `collider: false` leaves it off, which saves a call per object for geometry that is only meant to be looked at |
...
| `gameobject_add_component` | unsafe | Add a component by type name. `values` sets its serialized properties through the same writer as `inspect_write`, and the reply carries them in `written`. Adding the component and setting its values share one undo step. If a property is refused, the whole addition is undone, including any required components it brought with it |
...
| `inspect_write` | unsafe | Write a serialized property. Property paths and keys in `values` also accept unambiguous C# names. Without `component_type`, it tries the GameObject first, then a component that uniquely matches every property. Several matching components, or values split across components, are refused. `object_paths` makes the same edit on up to 500 objects in one undo step, and writes nothing if any of them fails to resolve. `asset_path` writes the contents of a prefab or the main asset at that path. For a prefab child, give `object_path` relative to the prefab root, leaving out the root name. `component_type` takes a short or full type name. An asset write reports `saved: true` only after the save is confirmed. For each property, `overriddenBy` lists up to 50 instances in loaded scenes whose overrides keep the asset edit from reaching them |
...
| `asset_export_package` | unsafe | Export the given assets to a `.unitypackage`. Each asset goes in with its `.meta`, so importing the file back restores the GUIDs and the references that pointed at them survive — which is what makes it a rollback point to take before a bulk edit that rewrites materials or re-slices sprites. The file lands outside the project, so `destination` is required, and one that is already there is kept unless `overwrite` is passed. The package is written beside the destination and moved into place, so a failed export leaves whatever was there before it intact. `include_dependencies` follows the whole reference graph, where one character prefab can reach most of the project. The reply reports the size actually written |
...
| `play_mode_play` | unsafe | Enter play mode. Unless Enter Play Mode Settings has Reload Domain off, the domain reloads and the connection briefly drops. `paused` defaults to false; pass true to start paused after one completed frame. The deferred reply carries the requested pause state in `paused` |
...
| `play_mode_step` | unsafe | Step forward, pausing first if needed. `count` advances 1 to 1000 frames in one call, and the reply carries the frame it reached and how far into the run that is. `paths` reads whatever else you were going to look at while the frame is still that one, in the form `reflect_read` takes. `animators` reports what each named object's Animator is doing at that frame — the state per layer, how far through it is, whether it is in transition, and the parameter values — because a state's progress is not a property `paths` can reach. Outside play mode the call fails the same way, and carries no readings. Use `seconds` instead of `count` to advance by elapsed game time; with neither, it advances one frame. `max_frames` caps the work at 2000 frames by default. Pass `changes: true` with 1 to 20 `paths` to record their values after each frame. Changes within a frame appear once. Each path keeps up to 200 changes; beyond that it keeps the first 199 and the last, with `truncated: true`. A path that cannot be read reports its first error, and a destroyed object reads as null. `stopReason` says what ended the run: `count`, `seconds`, `max_frames` or `play_mode_ended` |
...
| `render_capture_ab` | unsafe | Capture the frame, capture it again with the named objects hidden, and report what changed together with how much changes on its own. Two captures of an unchanged scene are not identical - animation, water, particles and temporal anti-aliasing all move between frames - and a change smaller than that movement means nothing, which looking at the two pictures cannot tell you. The objects go back before the call answers, and also if it is cancelled or the domain reloads; hiding them from `execute_code` leaves them hidden when the Editor stops. Writes three PNGs under `save_path_prefix`. `settle_frames` is how many Editor frames pass after the scene changes before it is captured |
| `render_capture_buffer` | unsafe | Save one of the pipeline's intermediate targets as a picture: `depth`, `normals`, `motion` or `opaque`. This is what a foam, fog or outline pass actually reads, and it is not always the silhouette on screen. Depth comes back as grey shading plus the nearest and farthest distance in metres, so a pixel can be read as a number; grey 0 is the far plane, where nothing was drawn. A buffer the pipeline is not producing is refused by name with the setting that turns it on, because captured anyway it would be a picture of zeroes - which for depth reads as an empty scene. Scriptable pipelines only |
| `render_profile_frame` | safe | Where a frame's time and its garbage go, sampled over a window of frames. CPU total, main thread, render thread and GPU, each as a median and a worst frame, plus the bytes allocated per frame and how many allocations. Draw call and triangle counts are not here - `render_stats` reports the last frame's. `markers` samples your own ProfilerMarkers alongside. **The numbers mean something in play mode**; in edit mode they describe the Editor drawing its own windows, and `playing` in the reply says which was measured. The main thread, render thread and GPU times come back only when the frame timing manager fed them; a counter that reported nothing is named in `silent` rather than given as zero (it needs Frame Timing Stats in Player Settings, which in the Editor is necessary without being sufficient - a built player is where these read true) |
...
| `shader_batching_check` | safe | Whether the SRP Batcher can keep a shader's material data on the GPU, and what stops it when it cannot. This answers "why are there still thousands of SetPass calls": **one incompatible shader breaks the batch for everything drawn with it**, and nothing in the Editor reports it outside the Shader Inspector, one shader at a time. The reason names the offending shader variable, which is what has to be moved or declared - for example `UnityPerMaterial var is not declared in shader property section (_Downscaling)`. Pass `scope: scene` for everything the open scenes draw with, or `path` / `name` / `object_path` for one. **Refused on the built-in pipeline**, which has no batcher to be compatible with. A shader with no subshader (it failed to compile) is reported as unchecked rather than checked |
| `material_read` | safe | A material's current values, keywords and render queue. Name a material asset with `path`, or a scene object with `object_path` to read every material slot on its renderer. Reading every slot reports each material's property count rather than its values. One renderer can carry dozens of materials of a few hundred properties each, so name a `slot` to get that slot's values. Pass a string as `property` to report only the properties whose name contains it; on a shader like lilToon, checking one property goes from 32,743 bytes to 653. A shader that is missing or unsupported is named in `shaderProblem`, which is the magenta case. A material that is not an asset comes back with a null path rather than being left out. `group` reports one description per distinct material with the objects that share it listed beside it, which is the answer to "are these the same material" without three hundred copies of it (measured 104,142 bytes against 3,213). Grouping is by content: neither the instance nor the name decides it, so materials set up the same way group together and their names come back in `names`. Several objects go in `object_paths`, up to 50 at a time; one that cannot be read carries its own error and the rest still come back. Three hundred objects read one at a time cost three hundred calls and 744 KB |
...
| `gpu_readback` | unsafe | Read a buffer or texture back and report `min`, `max`, `mean`, `zeroCount`, `allZero` and a histogram, plus `samples` raw values. A texture is read as a single 32-bit channel, so the numbers describe red alone |
...
| `timeline_set_track` | unsafe | Mute, lock, rename, or bind a track. Resolves the component the track's type wants, for example an Animator for an animation track. A binding is scene data on the PlayableDirector, so the `.playable` is left alone and `scene_save` is what persists it; mute, lock and rename live in the asset and do save the `.playable` |
...
| `reflect_read` | unsafe | Read live state by type and member path, private members included. A getter such as `Renderer.material` instantiates the shared asset, so prefer `sharedMaterial`. Read several at once with `paths` (50 at most): the reply keys each result by the path asked for, and one that cannot be read carries its own `error` without costing the others. A Collider's `bounds` is synced to the Transform before it is read |
| `reflect_find_type` | safe | Find a loaded type by name |
| `execute_code` | unsafe | Compile and run a C# snippet (the last resort when no tool reaches it). `UnityEngine.SceneManagement` and `UnityEditor.SceneManagement` are already imported, and `Object` names `UnityEngine.Object`. Compile errors give the line and column in the snippet. `McpSnippet.PathOf(GameObject)`, `McpSnippet.Find(string)`, `McpSnippet.IdOf(Object)` and `McpSnippet.All<T>(bool includeInactive = false)` need no extra imports and work on both supported Editor generations |
...
| `build_report` | safe | What the last build contained and what it weighed, read back from the report Unity writes beside the Library. **Nothing else could see inside a build** - `build_player` produces one and then loses sight of it. The reply gives the summary, a row per asset type, the heaviest individual assets with their paths, and the slowest build steps. Pass `compare_with` and a second report for **what changed between two builds**: which assets appeared, which grew, and by how much. That is the only form of the question that answers "why did this get bigger". Reading the report rather than the build folder means the answer survives the output being deleted |
| `build_switch_target` | unsafe | Switch the active target (reimports assets) |
| `project_settings` | unsafe | Read and change the settings under `ProjectSettings/`. The 14 sections are player, quality, graphics, tags, physics, physics2d, time, audio, input, editor, build, memory, navigation and presets. Give `property` a serialized name such as `m_Gravity`; a name that is not a whole path is searched for as a substring, descending into arrays and structs so that `m_QualitySettings.Array.data[0].shadowDistance` is findable. A listing stays at the top level and carries `truncated` when it stops at 200. A write is undoable and reaches disk through `AssetDatabase.SaveAssets`, which writes every other unsaved asset in the project with it; there is no call that writes one settings file on its own, and the reply says so. `properties` reads several of a section's properties in one call and `values` changes several together; a batched write applies nothing at all if any path is missing, because a layer collision pair written half-way leaves the matrix disagreeing with itself |
...
- Deletion is normally recoverable, so it asks for no confirmation. The three tools that do ask are `prefab_apply`, `editor_dialog_press` and `package_resolve`, because none of them can be undone.
...
- `execute_code` places the snippet in a method body, so a using directive there is a compile error. `System`, `System.Collections`, `System.Collections.Generic`, `System.Linq`, `System.Threading.Tasks`, `UnityEngine`, `UnityEditor`, `UnityEngine.SceneManagement` and `UnityEditor.SceneManagement` are already imported, and `Object` names `UnityEngine.Object`. Write any other type in full, as in `UnityEngine.Rendering.Volume`.
- `execute_code` is not on the undo stack. Use the authoring tools for authoring.

An argument that takes a JSON object also accepts that object encoded as a string, provided it is strict JSON. A vector argument written as `{x, y, z}` also takes comma-separated numbers, as in `"1,2,3"`, or an array of two to four numbers, in `x`, `y`, `z`, `w` order. Other text is refused. Property values still need the JSON type their property takes.

A scene object path without a leading `/` is tried from the scene roots first. If that finds nothing, it is tried as an exact object name, including inactive objects. One match is accepted; several are refused, with up to ten matching paths in the error.
