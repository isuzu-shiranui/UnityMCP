...
座標は、タブバーの下にあるウィンドウのコンテンツ領域の左上を原点とする point です。`normalized: true` の 0〜1 と、座標が範囲内かの判定は、タブバーを含むウィンドウ全体を基準にします。そのため正規化の 0.5 は、コンテンツの中心よりわずかに下にずれます。位置を厳密に決めたいときは point で渡してください。

ドラッグは `steps` と `frames_per_step` で、Editor の複数フレームに分けて送れます。どちらも上限は 1000 です。時間経過に反応する処理は、複数フレームに分けて送られること自体を検知して動きます。1 フレームにまとめて送ると再現しません。
