...
トークンを再生成した場合は、そのプロジェクトで `isuzu-unity-cli setup --mcp` を実行して、MCP クライアントを登録し直してください。`doctor --fix` は、どのプロジェクトの登録か確かめられない設定を書き換えません。
...
## 「No running Editor has ... open」で停止するとき

CLI のコマンドと MCP サーバーは、最初に選んだプロジェクトのパスにだけ再接続します。そのパスを開いている Editor が無いときは、別のプロジェクトの Editor が開いていても接続せずに停止します。

- そのプロジェクトを開き直してください。
- 別のプロジェクトを使うときは、CLI ならコマンドを `--project` を付けて実行し直します。MCP クライアントなら MCP サーバーを再起動します。Claude Desktop では、拡張機能をオフにしてからオンにします。
- 対象のプロジェクトを開いているのに断られるときは、パスの書き方が変わっていないか確認してください。ジャンクションや `subst` を経由して開いた場合や、大文字と小文字を変えた場合は、別のパスとして扱います。

作業ディレクトリが Unity プロジェクトの中にあり、そのプロジェクトを開いている Editor が無いときも、同じように停止します。
