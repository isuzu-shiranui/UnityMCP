...
バンドルには CLI 本体が入っているため、新しいリリースが出ても自動では更新されません。Claude Desktop 側にも通知は出ません。新しい `isuzu-unity-cli.mcpb` をダウンロードし、上書きで導入してください。入力したプロジェクト名はそのまま残ります。
...
- **クライアントはツールの説明文を毎回の要求で払います。** 全ツールで約 43,000 トークン、`diagnostics` だけなら約 11,000 トークンです。
