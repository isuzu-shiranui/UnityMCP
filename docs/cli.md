...
一度選択すると、`verify`、`jobs --wait`、`mcp-stdio` は同じプロジェクトパスにだけ再接続します。ポート、トークン、製品名の変更は受け入れますが、同名の別プロジェクトには切り替えません。そのパスの Editor が見つからない場合や複数ある場合も同様です。別のプロジェクトを選ぶには、コマンドを新しく起動してください。
